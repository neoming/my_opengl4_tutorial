...
bool screencapture() {
  ...
  if ( !stbi_write_png( name, g_gl_width, g_gl_height, 3, last_row, -3 * g_gl_width ) ) { fprintf( stderr, "ERROR: could not write screenshot file %s\n", name ); }
  free( buffer );
  return true;
}
...
int main() {
  ...
  GLuint shader_programme = create_programme_from_files( "src/10_screen_capture/test_vs.glsl", "src/10_screen_capture/test_fs.glsl" );
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
  ...
  // load texture
  GLuint tex;
  ( load_texture( "src/10_screen_capture/skulluvmap.png", &tex ) );
